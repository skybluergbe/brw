...
from bacpypes3.primitivedata import ObjectIdentifier, Real, Null, Tag
...
async def write_single_value(target_device, object_id, property_id, value, priority=16):
    ...
    try:
        ...
        if value is None:
            #bacnet_value = Any(Null())   # Null 값 (Priority Release)
            bacnet_value = Any(ApplicationTag(Null()))
        else:
            bacnet_value = Any(Real(value))  # 실수값
            ...
            if new_value is not None:
                ...
                if value is not None:
                    try:
                        ...
                        print(f"! 확인 비교 실패: {new_value}")
                else:
                    print("✓ 값이 Null로 해제되었습니다.")
            else:
...
async def main():
    ...
    # 예: 값 쓰기
    #await write_single_value(target_device, object_id, property_id, 43, priority=2)

    # 예: Priority Release (Null 쓰기)
    await write_single_value(target_device, object_id, property_id, None, priority=2)
