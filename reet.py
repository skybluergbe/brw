...
async def write_property(app, device_address, object_id, property_id, value, priority=None):
    ...
    try:
        ...
        if value is None and priority is not None:  # Null 값 (relinquish)
...
async def set_override(app, device_address, object_id, method="outOfService", value=None, priority=8):
    ...
    try:
        if method == "outOfService":
            ...
            # NULL 값 쓰기
            success = await write_property(app, device_address, object_id, "presentValue", None, priority)
...
async def restore_normal(app, device_address, object_id, method="outOfService"):
    ...
    try:
        if method == "outOfService":
            ...
            # 모든 수동 우선순위 해제 (8번 우선순위 해제)
            print(f"\n== 수동 override 해제 (우선순위 8) ==")
            
            # NULL 값 쓰기
            success = await write_property(app, device_address, object_id, "presentValue", None, 8)
            
            if success:
                print(f"성공: {object_id}.presentValue 우선순위 8 해제됨")
...
async def manage_override(target_device, object_id, action="status", method="priority", value=None, priority=8):
    ...
    try:
        ...
        if action == "set":
            ...
            # 특정 우선순위 해제
            await set_override(app, target_device, object_id, "relinquish", None, priority)
...
async def main():
    ...
    object_id = ("analogOutput", 1)  # analogoutput 객체

    # override 설정 방식
    # "status" - 상태만 조회
    # "set" - override 설정
    # "clear" - override 해제
    # "relinquish" - 특정 우선순위 해제
    action = "relinquish"
    
    # override 방식
    # "outOfService" - outOfService 속성 사용
    # "priority" - 우선순위 배열 사용
    method = "priority"
    
    # 설정할 값과 우선순위
    value = 2  # 설정할 상태 값
    priority = 1  # 우선순위 (1-16, 일반적으로 8번은 수동 조작용)
    
    # 실행
    await manage_override(target_device, object_id, action, method, value, priority)

if __name__ == "__main__":
    print("BACnet Override 관리 도구")
    print("========================")
    asyncio.run(main())