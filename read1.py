...
async def simple_read():
    ...
    try:
        ...
        # 디버깅: 원본 객체 타입도 확인
        raw_response = await app.request(ReadPropertyRequest(
            objectIdentifier=ObjectIdentifier(object_id),
...
if __name__ == "__main__":
    ...
    print("실행 옵션:")
    print("1. asyncio.run(main())              # 전체 예제")
    print("2. asyncio.run(simple_read())       # 간단한 예제") 
    print("3. asyncio.run(batch_example())     # 배치 읽기")
    print("4. asyncio.run(discover_devices())  # 디바이스 검색")
    print()
    print("주의사항:")
